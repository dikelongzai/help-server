...
public interface AppServerMapper {
    ...
    @Select("select create_date,last_update,state,order_id,order_type,recharge_order,recharge_phone,recharge_uid,withdrawals_order" +
            ",withdrawals_phone,withdrawals_uid,money_num,complaint_status,remittance_url,from_date,to_date,match_date,confirm_date,order_num,payment_date from orders where order_type = #{ordertype} AND state <> 'D' AND (recharge_uid = #{rechargeuid} OR withdrawals_uid =#{rechargeuid})")
    public List<Orders> getOrderInfo(@Param("ordertype") int ordertype, @Param("rechargeuid") Long rechargeuid);

    @Select("select create_date,last_update,state,order_id,order_type,recharge_order,recharge_phone,recharge_uid,withdrawals_order" +
            ",withdrawals_phone,withdrawals_uid,money_num,complaint_status,remittance_url,from_date,to_date,match_date,confirm_date,order_num,payment_date from orders where order_type <> 2 AND state <> 'D' AND (recharge_uid = #{rechargeuid} OR withdrawals_uid =#{rechargeuid})")
...
//
    ...
    //只更新offer_help 状态，确认收款，确认付款更新
    @Update("update offer_help set help_status = #{helpstatus} where help_order = #{helporder}")
    public int updateOfferHelpStatus(@Param("helpstatus") int helpstatus,@Param("helporder") String helporder);
    ...
    @Select("SELECT count(1) FROM  offer_help where user_id =#{uid} and help_type = 1 and wallet_type = 0 and help_status <>2 and help_status <>8 and help_status <>7")
    public int getDynamicOfferHelpCountIncome(@Param("uid") long uid);
